...
data class JsonNodeObject(var fieldMap: Map<String, JsonNode>) : JsonNode()
...
object JsonUser : JsonF<User> {
    ...
    val id = JsonProp("id-ss", JsonInt)
    val name by JField(JsonString)

    override fun from(node: JsonNode): Outcome<JsonError, User> =
        when (node) {
            is JsonNodeObject -> ::User `=` id.fromObj(node) `+` name.fromObj(node)

            else -> JsonError(node.toString(), "Expected json object").asFailure()
        }

    override fun toJson(value: User): JsonNode =
        createJsonNode(
            id.getFrom(value.id),
            name.getFrom(value.name)
        )

    fun createJsonNode(vararg fields: Pair<String, JsonNode>): JsonNode =
...
data class JsonProp<T : Any>(val propName: String, val jf: JsonF<T>) {
    ...
    fun getFrom(value: T): Pair<String, JsonNode> =
